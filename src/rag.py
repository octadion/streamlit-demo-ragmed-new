...
def setup_graph():
    ...
    def generate_paraphrases_cached(query:str, n:int):
        ...
        prompt = multi_query_paraphrasing_prompt.format(
            n=N_PARAPHRASES, question=query
        )

        # generate multi query paraphrase
        resp = llm.invoke(prompt)

        # Extract into tuple of paraphrases
        ...
        # if paraphrases is empty, use the original question
        if not paraphrases:
            paraphrases = [query]
    ...
    def cached_retrieve_for_paraphrase(retriever_name: str, paraphrase: str):
        ...
        # If retriever info not found, use baseline database
        if retriever_info is None:
            print("⚠️ No retriever found, falling back to Baseline DB")
            retriever_info = next(r for r in retrievers if r["name"] == "Base")
    ...
    def route_question(question: str):
        ...
        # Multi vector databases routing
        ...
        # If there no database relevant, use Baseline database
        if retriever_info is None:
    ...
    def generate(state):
        ...
        relevant = state.get("documentarth_relevant", True)
